...
impl WebauthnVerifier {
    ...
    ///
    /// Validates:
    /// 1. Client data JSON (challenge, origin, type)
    /// 2. Attestation statement signature
    /// 3. Attestation certificate chain → trusted root
    /// 4. RP ID hash matches
    /// 5. User verification flag
    pub fn verify_registration(
        ...
        Err(ConclaveError::Unsupported(
            "WebAuthn verify: add `webauthn-rs` or `passkey-rs` crate".into(),
        ))
    }
    ...
    /// Validates:
    /// 1. Client data JSON (challenge, origin, type)
    /// 2. Assertion signature against credential public key
    /// 3. Sign counter increment
    ...
    pub fn verify_authentication(
        ...
        Err(ConclaveError::Unsupported(
            "WebAuthn verify: add `webauthn-rs` crate".into(),
        ))
    }
