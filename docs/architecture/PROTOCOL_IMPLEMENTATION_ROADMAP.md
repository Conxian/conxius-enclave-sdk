...
| Current code boundary | `src/protocol/ark.rs` provides typed version, VTXO/outpoint/round/server/connector/forfeit/operation IDs, backend/state/recovery/exit models, and structural tree/descriptor validation. Generic seed/index APIs, public-key derivation, recovery, tree construction, and forfeit/settlement signing return exact `ProtocolUnsupported`. |
