...
- **#267 BitVM2 Groth16 Verification**: Implemented BLS12-381 compressed point decoding and structural validation in `BitVm2Groth16Verifier`. Valid proof structures return `Groth16VerificationOutcome::Valid`, invalid/corrupted points return `Invalid`, and malformed inputs fail closed with boundary error. *(Superseded by the Session 61 real pairing verification below — the structural-only path was a fail-open gap.)*

### Session 61 Resolution (2026-08-29)
- **#267 BitVM2 Groth16 Verification (real pairing)**: Replaced the structural-only verifier with a real BLS12-381 Groth16 pairing check behind the new `groth16` feature (`bls12_381 = "0.8"`). `BitVm2Groth16Verifier::verify` now runs `e(A,B) == e(alpha,beta)·e(acc,gamma)·e(C,delta)` and fails closed on decompression/identity/subgroup/arity failures. Without the `groth16` feature it returns `VerificationUnavailable` (never `Valid`). Public inputs map one-to-one to four `Fr` scalars (`instance_id`, `commitment_id`, `state_root_hash`, `challenge_digest`) via `Scalar::from_bytes_wide`; the verification key must carry exactly `1 + 4` IC terms. Added `groth16_verifier_verifies_genuine_proof`, `groth16_verifier_rejects_arity_mismatch`, and `groth16_verifier_rejects_arbitrary_bytes_fail_closed` tests. This closes the Groth16 verifier gap (DEBT `PROTO-001` sub-item) but does **not** establish BitVM2 protocol-conformance, live bridge, independent-review, or production-support evidence (`#202` remains open).
- **G240-RP Durable Replay (reference backend)**: Implemented `FileBackedDurableReplayStore` (gated `not(wasm32)`) — a real durable consume-once backend. `OpenOptions::create_new(true)` (`O_EXCL`) provides the atomic conditional-write primitive equivalent to `INSERT … ON CONFLICT DO NOTHING`; records are `fsync`-ed before `Consumed` is returned, a failed write yields `UncertainCommit` (fail closed), and a persisted high-water clock resists rollback across restarts. Tests prove restart durability, idempotent-retry/conflict, expiry/rollback fail-closed, and outage. This satisfies the "durable across restarts" acceptance item with a local reference backend; the multi-replica (DynamoDB/PostgreSQL) backend remains outside the crate per the `DurableReplayStore` contract, and full `#240` closure still requires the trust/collateral operational items plus `#202` review evidence.

### Session 66 Resolution (2026-09-03)
- **PROTO-001 Fedimint DLEQ Proof Integration**: Wired `DleqProof::verify`, `FedimintAdapter::create_dleq_proof`, and `FedimintAdapter::create_blind_signature_request` in `src/protocol/nexus/fedimint.rs` to the real BLS12-381 `fedimint_crypto` backend under `#[cfg(feature = "fedimint-crypto")]`. Verified that with `fedimint-crypto` enabled, DLEQ proof creation, verification, and blind signature requests run through the real BLS12-381 DLEQ crypto, and without the feature they fail closed returning `ProtocolUnsupported`.

### Session 62 Resolution (2026-08-29)
...
### Session 73 Resolution (2026-09-10)
- **SDK-005 Babylon BTC Staking Protocol Hardening**: Added Extractable One-Time Signatures (EOTS) commitment hash calculation (`BabylonEotsCommitment`), signature verification (`BabylonEotsSignature::verify`), double-signing detection at matching height and round, and algebraic slashing secret key extraction (`extract_slashing_key`) in `src/protocol/babylon.rs`.
- **SDK-006 RGB Client-Side Validated Asset Protocol Hardening**: Added blinded single-use seals (`RgbBlindedSeal`) via SHA-256 tagged hash $H(\text{txid} \parallel \text{vout} \parallel \text{blinding\_factor})$, seal unblinded verification (`verify`), asset allocation commitments (`RgbAssetAllocation`), and batch asset transfer transition signing through UCS (`RgbBatchTransition`) in `src/protocol/rgb.rs`.
- **End-to-End Gap Scorecard Alignment**: Verified end-to-end multi-chain protocol support status. All 600 unit tests and 4 integration test suites pass with 0 warnings or failures.
...
### Session 76 Resolution (2026-09-20)
- **Vendor-Backed Production Hardware Attestation**: Replaced simulated hardware attestation evidence with production hardware attestation across AWS Nitro Root G1 DER verifier (`src/enclave/verifiers/nitro_verifier.rs`), Android StrongBox KeyMint authorization verification (`src/enclave/android_authorization.rs`), and PKCS#11 verifier (`src/enclave/verifiers/pkcs11_verifier.rs`).
- **FROST DKG (RFC 9591) Un-quarantined & Nonce Handling Completed**: Completed RFC 9591 FROST DKG Round 1 & Round 2 cryptographic share verification, Proof of Knowledge verification, single-use nonce lifecycle enforcement (`self.nonces_map.remove`), and ZF FROST threshold signing in `src/protocol/frost.rs` and `src/protocol/frost_crypto.rs`.
