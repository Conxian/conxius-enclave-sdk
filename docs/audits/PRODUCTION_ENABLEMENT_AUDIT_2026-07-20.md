...
| --- | --- | --- | --- |
| P2-01 | **Executable example wording overstated trust-tier enforcement (resolved on this branch).** | `examples/attestation_verification.rs:8-29` now labels CloudTEE and StrongBox as conditional vocabulary with provider verifiers unavailable, and states that high-value operations remain blocked until provider-verified support is integrated. | The stale production-label concern is resolved in the current example. No provider-backed production support is established; provider integration and evidence remain open gates. |
