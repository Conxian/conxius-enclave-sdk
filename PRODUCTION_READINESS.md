...
Issue #240 adds a provider-neutral, versioned trust-bundle validator, explicit refresh/rollback states, an authenticated canonical digest/verifier boundary, a typed atomic `ReplayStore` contract, and a complete domain-separated replay binding. These are contract and local-test foundations only. The production registry still exposes unavailable provider routes; `ReplayGuard` remains process-local; no durable backend, vendor roots, hardware runtime evidence, distributed replay, independent review, SBOM, provenance, or exact release artifact is claimed. See [TRUST_REPLAY_FOUNDATION.md](./docs/architecture/TRUST_REPLAY_FOUNDATION.md).
...
- [x] Provider-neutral trust/replay contract foundation and focused fail-closed tests recorded in [TRUST_REPLAY_FOUNDATION.md](./docs/architecture/TRUST_REPLAY_FOUNDATION.md); durable provider-backed replay and value-bearing deployment evidence remain open
